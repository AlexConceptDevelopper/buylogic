...
import { createSupplier, getSuppliers, updateSupplier } from "../api/supplier.api";
...
export default function SuppliersPage() {
  ...
  const { loading: updating, execute: executeUpdate } = useAsync<Supplier>();
  ...
  const handleUpdate = async () => {
    ...
    if (!trimmedName) {
      setEditError("Le nom du fournisseur est obligatoire.");
      return;
    }
    ...
    const updated = await executeUpdate(() =>
      updateSupplier(editingSupplier.idSupplier, payload),
    );

    if (!updated) {
      setEditError(
        "Impossible de modifier le fournisseur. Vérifiez les informations.",
      );
      return;
    }
    ...
    setSuppliers((current) =>
      current.map((s) =>
        s.idSupplier === updated.idSupplier ? updated : s,
      ),
    );
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filteredSuppliers.map((supplier) => (
            <Link
              ...
              className="group relative cursor-pointer rounded-2xl border border-white/5 bg-slate-900/70 p-5 transition hover:border-cyan-400/20 hover:bg-slate-900 flex flex-col justify-between"
            >
              <div>
              ...
              <div className="mt-6 flex items-center justify-between pt-4 border-t border-white/5">
                <button
                  ...
                  className="cursor-pointer rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-slate-300 transition hover:bg-white/10 hover:text-white"
                >
                  Modifier
                </button>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Mettez à jour les informations ou activez/désactivez le fournisseur.
                </p>
              </div>
